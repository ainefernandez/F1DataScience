...
monacoCarlos<-read.csv("carlosmonaco.csv")
monCarlos<-ggplot(monacoCarlos, aes(x=LapNumber, y=LapTimeSeconds, col=Driver)) +
  geom_line(size=1) +
  geom_point(size=3)+
  scale_color_manual(values=c("HAM"="#6CD3BF","OCO"="#2293D1","SAI"="#F91536"))+
  scale_y_continuous(limits=c(80,110),breaks=seq(80, 110, by = 5))+
  scale_x_continuous(breaks=seq(30,35, by = 1))+
  theme_bw()+
  labs(x = "Lap", y = "Lap Time (s)",title="VER, NOR and SAI laptimes comparison",subtitle ="2023 Monaco GP (56-78)",caption = "@f1.datascience" )+
  theme(panel.background=element_rect(fill="white", color="white"),
        plot.background=element_rect(fill="white", color="white"),
...
laptime_data <- data.frame(
  Lap = rep(30:35, 3),                      # Lap numbers
  Driver = rep(c("OCO", "SAI", "HAM"), each = 6),  # Car names
  LapTime = c(77.334, 77.331, 94.668, 79.289, 76.950, 77.095,   # Lap times
...
L1<-filter(monacorain,monacorain$Lap==51)
L2<-filter(monacorain,monacorain$Lap==52)
L3<-filter(monacorain,monacorain$Lap==53)
L4<-filter(monacorain,monacorain$Lap==54)
L5<-filter(monacorain,monacorain$Lap==55)

p2<-ggplot(L5, aes(x = X, y = Y)) +
  geom_path(size = 8, linetype = "solid", color = "black") +
  ...
  labs(title = "Slicks vs Intermediates",subtitle="2023 Monaco GP",caption = "@f1.datascience")+theme(legend.position = "none")
p2
