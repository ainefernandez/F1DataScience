...
LECQ <- read.csv("LECQMiami.csv")
VERQ <- read.csv("VERQMiami.csv")
SAIQ <- read.csv("SAIQMiami.csv")
VERQ$faster <- NA

for (i in 1:nrow(VERQ)) {
  if (!is.na(LECQ$Speed[i]) & !is.na(SAIQ$Speed[i]) & !is.na(VERQ$Speed[i])) {
    if (VERQ$Speed[i] > LECQ$Speed[i] & VERQ$Speed[i] > SAIQ$Speed[i]) {
      VERQ$faster[i] <- "Verstappen is faster"
    } else if (SAIQ$Speed[i] > VERQ$Speed[i] & SAIQ$Speed[i] > LECQ$Speed[i]) {
...
pSAIQ <- ggplot(LapDataVERJAP, aes(x = X, y = Y)) +
  ...
  scale_color_manual(
    ...
    breaks = c("Verstappen is faster", "Leclerc is faster", "Sainz is faster")
  ) +
  ...
  labs(title = "VER, LEC, and SAI fastest lap comparison", subtitle = "2024 Miami GP", caption = "@f1.datascience")

pSAIQ

#Race

results<-read.csv("ResultsMiamiSprint.csv")
...
p <- ggplot(results) + 
  ...
  theme(panel.background = element_blank(), 
        ...
        legend.text=element_text(size=13),
        legend.title=element_text(size=13))+
  labs(title="Grid and race position",subtitle="2024 Miami GP (Sprint)",caption = "@f1.datascience")
p

gapSingaporeQ<-read.csv("GapRacePaceMiamiSprint.csv")
...
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Team,-Gap),fill=Team))+ 
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks.y =element_blank())+
  labs(caption = "@f1.datascience",subtitle = "2024 Miami GP (Sprint)")+theme(legend.position = "none")
pQualyS

gapSingaporeQ<-read.csv("GapMiami.csv")
...
gapSingaporeQ<-read.csv("idealgapMiami.csv")
...
gapSingaporeQ<-read.csv("bestsectorMiami.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
...
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector2,y=reorder(Driver,-GapSector2),fill=Driver))+ 
  geom_vline(xintercept = seq(0, .5, by = .05), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
...
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector3,y=reorder(Driver,-GapSector3),fill=Driver))+ 
  geom_vline(xintercept = seq(0,.4, by = .05), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
  scale_x_continuous(breaks = seq(0, .4, by = .05))+
  scale_y_discrete(expand = c(0,0))+
...
singapur<-read.csv("LaptimesMiami.csv")
drivercol<-c("NOR" = "#F58020","VER"="#3671C6","LEC"="#F91536")
monlando <- ggplot(singapur, aes(x = LapNumber, y = LapTime,color=Driver)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  scale_y_continuous(limits = c(90.8, 92.8), breaks = seq(90.8, 92.8, by = 0.2)) +
  scale_x_continuous(breaks = seq(33, 57, by = 1)) +
  scale_color_manual(values = drivercol)+
  theme_bw() +
  labs(
    x = "Lap",
    y = "LapTime",
    title = "NOR,VER,LEC and HAM laptimes comparison",
    subtitle = "2024 Miami GP (33-57)",
