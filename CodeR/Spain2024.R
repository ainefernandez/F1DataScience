...
VERQ <- read.csv("VERQSpain.csv")
HAMQ <- read.csv("HAMQSpain.csv")
NORQ <- read.csv("NORQSpain.csv")
NORQ$faster <- NA
...
for (i in 1:nrow(NORQ)) {
  if (!is.na(VERQ$Speed[i]) & !is.na(NORQ$Speed[i]) & !is.na(HAMQ$Speed[i])) {
    if (HAMQ$Speed[i] > VERQ$Speed[i] & HAMQ$Speed[i] > NORQ$Speed[i]) {
      NORQ$faster[i] <- "Hamilton is faster"
...
pSAIQ <- ggplot(LapDataVERJAP, aes(x = X, y = Y)) +
  ...
  scale_color_manual(
    values = c("Hamilton is faster" = "#6CD3BF", "Norris is faster" = "#F58020", "Verstappen is faster" = "#3671C6"),
    breaks = c("Norris is faster", "Verstappen is faster", "Hamilton is faster")
  ) +
  ...
  labs(title = "NOR, VER, and HAM fastest lap comparison", subtitle = "2024 Spanish GP", caption = "@f1.datascience")

pSAIQ


gapSingaporeQ<-read.csv("GapSpain.csv")
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Driver,-Gap),fill=Driver))+ 
  ...
  labs(caption = "@f1.datascience",subtitle = "2024 Spanish GP")+theme(legend.position = "none")
pQualyS




gapSingaporeQ<-read.csv("idealgapSpain.csv")
...
gapSingaporeQ<-read.csv("idealgapsTSpain.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
...
gapSingaporeQ<-read.csv("bestsectorSpain.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector1,y=reorder(Driver,-GapSector1),fill=Driver))+ 
  geom_vline(xintercept = seq(0, .6, by = 0.1), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
...
results<-read.csv("ResultsSpain.csv")
...
p <- ggplot(results) + 
  ...
  theme(panel.background = element_blank(), 
        ...
        legend.text=element_text(size=13),
        legend.title=element_text(size=13))+
  labs(title="Grid and race position",subtitle="2024 Spanish GP",caption = "@f1.datascience")
p

stints<-read.csv("StintsSpain.csv")
...
stints$Driver<-factor(stints$Driver,levels=c("SAR","TSU","ALB","MAG","BOT","RIC","STR",
                                             "ZHO","ALO","HUL","OCO","GAS",
                                             "PER","PIA","SAI","LEC","RUS",
                                             "HAM","NOR","VER"))
stintsSpain <- ggplot(stints, aes(Driver, StintLength, fill = Stint_Compound)) +
  ...
  labs(title = "Tyre strategies", subtitle = "2024 Spanish GP", caption = "@f1.datascience", y = "Lap") +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white", color = "white"),
        plot.background = element_rect(fill = "white", color = "white"),
        plot.title = element_text(size = 18, hjust = 0.5),
...
gapSingaporeQ<-read.csv("GapRacePaceSpain.csv")
