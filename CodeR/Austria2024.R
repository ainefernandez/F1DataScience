...
VERQ <- read.csv("VERQAustria.csv")
HAMQ <- read.csv("RUSQAustria.csv")
NORQ <- read.csv("NORQAustria.csv")
VERQ$faster <- NA
...
pSAIQ <- ggplot(LapDataVERJAP, aes(x = X, y = Y)) +
  ...
  scale_color_manual(
    values = c("Russell is faster" = "#6CD3BF", "Norris is faster" = "#F58020", "Verstappen is faster" = "#3671C6"),
    breaks = c("Norris is faster", "Verstappen is faster", "Russell is faster")
  ) +
  ...
  labs(title = "VER, NOR, and RUS fastest lap comparison", subtitle = "2024 Austrian GP", caption = "@f1.datascience")

pSAIQ


gapSingaporeQ<-read.csv("GapAustria.csv")
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Driver,-Gap),fill=Driver))+ 
  ...
  labs(caption = "@f1.datascience",subtitle = "2024 Austrian GP")+theme(legend.position = "none")
pQualyS




gapSingaporeQ<-read.csv("idealgapAustria.csv")
...
gapSingaporeQ<-read.csv("idealgapsTAustria.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
...
gapSingaporeQ<-read.csv("bestsectorAustria.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector1,y=reorder(Driver,-GapSector1),fill=Driver))+ 
  geom_vline(xintercept = seq(0, .4, by = 0.05), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
...
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector3,y=reorder(Driver,-GapSector3),fill=Driver))+ 
  geom_vline(xintercept = seq(0,.85, by = .05), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
...
results<-read.csv("ResultsAustria.csv")
results$GridPosition[results$Abbreviation=="ALB"]<-20
...
p <- ggplot(results) + 
  ...
  theme(panel.background = element_blank(), 
        ...
        legend.text=element_text(size=13),
        legend.title=element_text(size=13))+
  labs(title="Grid and race position",subtitle="2024 Austrian GP",caption = "@f1.datascience")
p

stints<-read.csv("StintsAustria.csv")
...
stints$Driver<-factor(stints$Driver,levels=c("NOR","SAR","ALO","ZHO","BOT","ALB","TSU",
                                             "STR","OCO","LEC","GAS","RIC",
                                             "MAG","PER","HUL","VER","HAM",
                                             "SAI","PIA","RUS"))
stintsSpain <- ggplot(stints, aes(Driver, StintLength, fill = Stint_Compound)) +
  ...
  labs(title = "Tyre strategies", subtitle = "2024 Austrian GP", caption = "@f1.datascience", y = "Lap") +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white", color = "white"),
        plot.background = element_rect(fill = "white", color = "white"),
        plot.title = element_text(size = 18, hjust = 0.5),
...
gapSingaporeQ<-read.csv("GapRacePaceAustria.csv")
names(gapSingaporeQ)<-c("Team","Gap")
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Team,-Gap),fill=Team))+ 
  geom_vline(xintercept = seq(0, 2, by =.2), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=teamscolors)+
