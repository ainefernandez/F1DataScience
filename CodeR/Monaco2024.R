...
PIAQ <- read.csv("PIAQMonaco.csv")
LECQ <- read.csv("LECQMonaco.csv")
SAIQ <- read.csv("SAIQMonaco.csv")
LECQ$faster <- NA

for (i in 1:nrow(LECQ)) {
  if (!is.na(PIAQ$Speed[i]) & !is.na(SAIQ$Speed[i]) & !is.na(LECQ$Speed[i])) {
    if (LECQ$Speed[i] > PIAQ$Speed[i] & LECQ$Speed[i] > SAIQ$Speed[i]) {
      LECQ$faster[i] <- "Leclerc is faster"
    } else if (PIAQ$Speed[i] > LECQ$Speed[i] & PIAQ$Speed[i] > SAIQ$Speed[i]) {
      LECQ$faster[i] <- "Piastri is faster"
    } else if (SAIQ$Speed[i] > LECQ$Speed[i] & SAIQ$Speed[i] > PIAQ$Speed[i]) {
...
pSAIQ <- ggplot(LapDataVERJAP, aes(x = X, y = Y)) +
  ...
  scale_color_manual(
    values = c("Leclerc is faster" = "#F91536", "Piastri is faster" = "#F58020", "Sainz is faster" = "gold"),
    breaks = c("Leclerc is faster", "Piastri is faster", "Sainz is faster")
  ) +
  ...
  labs(title = "LEC, PIA, and SAI fastest lap comparison", subtitle = "2024 Monaco GP", caption = "@f1.datascience")

pSAIQ


gapSingaporeQ<-read.csv("GapMonaco.csv")
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Driver,-Gap),fill=Driver))+ 
  ...
  labs(caption = "@f1.datascience",subtitle = "2024 Monaco GP")+theme(legend.position = "none")
pQualyS




gapSingaporeQ<-read.csv("idealgapMonaco.csv")
...
gapSingaporeQ<-read.csv("idealgapsTMonaco.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
...
gapSingaporeQ<-read.csv("bestsectorMonaco.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector1,y=reorder(Driver,-GapSector1),fill=Driver))+ 
  geom_vline(xintercept = seq(0, 1.2, by = 0.1), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
...
results<-read.csv("ResultsMonaco.csv")
...
p <- ggplot(results) + 
  ...
  theme(panel.background = element_blank(), 
        ...
        legend.text=element_text(size=13),
        legend.title=element_text(size=13))+
  labs(title="Grid and race position",subtitle="2024 Monaco GP",caption = "@f1.datascience")
p

stints<-read.csv("StintsMonaco.csv")
...
stints$Driver<-factor(stints$Driver,levels=c("MAG","HUL","PER","OCO","ZHO","SAR","STR",
                                             "BOT","RIC","ALO","GAS","ALB",
                                             "TSU","HAM","VER","RUS","NOR",
                                             "SAI","PIA","LEC"))
stintsSpain <- ggplot(stints, aes(Driver, StintLength, fill = Stint_Compound)) +
  ...
  coord_flip() +
  scale_y_continuous(limits = c(0,78), breaks = seq(0, 78, by = 2)) +
  ...
  labs(title = "Tyre strategies", subtitle = "2024 Monaco GP", caption = "@f1.datascience", y = "Lap") +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white", color = "white"),
        plot.background = element_rect(fill = "white", color = "white"),
        plot.title = element_text(size = 18, hjust = 0.5),
...
gapSingaporeQ<-read.csv("GapRacePaceMonaco.csv")
names(gapSingaporeQ)<-c("Team","Gap")
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Team,-Gap),fill=Team))+ 
  geom_vline(xintercept = seq(0, 2.6, by =.1), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=teamscolors)+
  scale_x_continuous(breaks = seq(0, 2.6, by =.1))+
  scale_y_discrete(expand = c(0,0))+
  geom_text(aes(label = round(Gap,3)), hjust = -0.1, size = 4) +
  xlab("Gap (seconds)") +
  ylab("Team") +
...
drivercolors <- c("VER" = "#3671C6", "PER" = "#FFD700", "ALO" = "#358C75", "SAI" = "#04C404", "HAM" = "#6CD3BF",
                  "STR" = "#358C75", "RUS" = "#6CD3BF", "BOT" = "#04C404", "GAS" = "#FF66C4", "ALB" = "#37BEDD",
                  "TSU" = "#0335D3", "SAR" = "#37BEDD", "MAG" = "#B6BABD", "DEV" = "#5E8FAA", "HUL" = "#B6BABD",
                  "ZHO" = "#04C404", "NOR" = "#F58020", "OCO" = "#FF66C4", "LEC" = "#F91536", "PIA" = "#F58020",
                  "RIC" = "#0335D3", "LAW" = "#5E8FAA", "BEA" = "#F91536")



singapur<-read.csv("top5points.csv")
singapur$GP<-factor(singapur$GP, levels = c("Bahrain","Saudi Arabia","Australia","Japan","China","Miami","Imola","Monaco"))
monlando <- ggplot(singapur, aes(x = GP, y = Points, color = Driver, group = Driver)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
