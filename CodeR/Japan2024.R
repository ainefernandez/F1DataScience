...
pQualyS<-ggplot(gapSingaporeQ, aes(x=Gap,y=reorder(Driver,-Gap),fill=Driver))+ 
  ...
  labs(caption = "@f1.datascience",subtitle = "2024 Japanese GP")+theme(legend.position = "none")
pQualyS




gapSingaporeQ<-read.csv("idealgapJapan.csv")
...
gapSingaporeQ<-read.csv("bestsectorJapan.csv")
#gapSingaporeQ<-gapSingaporeQ[gapSingaporeQ$Driver!="ZHO",]
pQualyS<-ggplot(gapSingaporeQ, aes(x=GapSector1,y=reorder(Driver,-GapSector1),fill=Driver))+ 
  geom_vline(xintercept = seq(0, 1.1, by = 0.1), linetype = "dashed", color = "grey")+
  geom_bar(stat="identity")+
  scale_fill_manual(values=drivercolors)+
...
results<-read.csv("ResultsJapan.csv")
...
p <- ggplot(results) + 
  ...
  theme(panel.background = element_blank(), 
        ...
        legend.text=element_text(size=13),
        legend.title=element_text(size=13))+
  labs(title="Grid and race position",subtitle="2024 Japanese GP",caption = "@f1.datascience")
p
...
gapSingaporeQ<-read.csv("GapRacePaceJapan.csv")
