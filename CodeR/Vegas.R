...
singapur<-read.csv("laptimesQVegas.csv")
monlando <- ggplot(singapur, aes(x = LapNumber, y = LapTime, col = FreshTyre)) +
  geom_point(size = 3) +
  scale_color_manual(
    values = c("False" = "#0098C3", "True" = "#F91536"),
    breaks = c("False", "True"),
    labels = c("Old Tyre", "New Tyre")
  ) +
  scale_y_continuous(limits = c(92.5, 97.5), breaks = seq(92.5, 97.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(1, 24, by = 1)) +
  theme_bw() +
  labs(
    x = "Lap",
    y = "Lap Time (s)",
    title = "Track evolution during Qualy",
    subtitle = "2023 Las Vegas GP",
    caption = "@f1.datascience",
    col = "Tyre Type"  # Set the legend title
  ) +
  theme(
